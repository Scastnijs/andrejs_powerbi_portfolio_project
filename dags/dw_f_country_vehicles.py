...
def task1(**context):
    ...
    records = hook.get_records("""
        select 
            COALESCE(ca.canonical_country, uv.country) AS country,
            value,
            indicator,
            unit_code,
            year
        from
            (select 
                country,
                per1kpeople as value,
                'car_density' as indicator,
                'THS_PER' as unit_code,
                year
            from staging.vehicles_country
            UNION
            select 
                country,
                total as value,
        ...
        LEFT JOIN staging.country_alias ca
            ON uv.country = ca.alias_country
    """)
